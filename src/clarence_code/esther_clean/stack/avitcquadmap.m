function avitcquadmap(years,coefp,coeft,tlat, tlon, loc, proj)
...
for j=1:20 %s  % loops through the years
...
%Todo: X,Y below are LON,LAT (respectively). P & T are derived from x,y (planar), but used as though they are associated with X,Y. Is this a correct mapping?
...
%   close(gcf);
	saveas(fig,strcat('file',num2str(years(j)),'.png'));
	close(fig);
end

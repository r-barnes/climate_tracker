function  xysoln=rtriquad(coefP, coefT, combo)
...
vsoln=[];
for t=1:numtrack
center=double(combo(t,1:2));
for i=1:numyear
    v=[coefP(:,i)'; coefT(:,i)'];
    locsoln = fsolveq(v, combo(t,3:4), center); %Todo: What does this return if an answer is not found?
%    vsoln(:,i)=locsoln;
    vsoln=[[vsoln];[locsoln]];
...
a = v(1, :);
b = v(2, :);
opts=optimset('TolFun', 1e-10, 'MaxIter', 1000, 'MaxFunEval', 2000, 'Display', 'off');

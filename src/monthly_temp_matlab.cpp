...
void store_monthly(float datum, char flag, int year, int month, char element, const char station_csn[]){
	...
	if(element!=CLIMATE_AVG_TEMP) return;
...
int main(int argc, char **argv){
	...
	opterr = 0;
	set_admissible_elements("0000001");
	FILE *fout;
	...
	do_output[CLIMATE_AVG_TEMP]=false;
	...
	if(!do_output[CLIMATE_AVG_TEMP]) return 0;
	string fname="";
	if(fname_prefix!=NULL){
		fname+=fname_prefix;
		fname+=".";
	}
	fname+=CLIMATE_NAMES[CLIMATE_AVG_TEMP];
